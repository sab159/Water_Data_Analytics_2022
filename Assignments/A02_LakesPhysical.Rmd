...
```{r}
lakes_temp <- ggplot(NTLdata, aes(x = temperature_C, y = depth, color = daynum)) +
              scale_y_reverse() +
              geom_point(alpha = 0.5) + 
              scale_color_viridis_c() + 
              labs(title = "Lake temperature profiles of NTL_LTER lakes",
...
#WRITE THIS!!!
# OBSERVATIONS ON SEASONAL TRENDS: 
```

```{r}   
lakes_DO <- ggplot(NTLdata, aes(x = dissolvedOxygen, y = depth, color = daynum)) +
            scale_y_reverse() +
            geom_point(alpha = 0.5) + 
            scale_color_viridis_c() + 
            labs(title = "Lake temperature profiles of NTL_LTER lakes",
                 x = "DO", y = "Depth (m)", color = "DOY") +
            facet_wrap(~ lakename)
#what are the units on DO? 

lakes_DO

#WRITE THIS!!!
# OBSERVATIONS ON SEASONAL TRENDS: 
# Tuesday Lake has some WONKY values that really throw of the scaling... Paul Lake also has one.
```
