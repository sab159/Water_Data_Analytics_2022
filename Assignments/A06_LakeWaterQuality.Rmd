...
*  **MeasureQualifierCode** Contains information about data flags: 
  ...
  * *J* designates above detection limit but below quantification limit (action: retain value)
...
class(NCdata$ActivityStartDate) #already a date, no need to change or cast
...
# Format and summarize cleaned data
NCdata_summary <- NCdata_processed %>% select(OrganizationIdentifier, OrganizationFormalName, 
                                              ActivityStartDate, ActivityConductingOrganizationText,
                                       ...
                                       group_by(OrganizationIdentifier, OrganizationFormalName, 
                                                ...
                                                MonitoringLocationIdentifier,
                                                Depth_m, Variable) %>%
                                       summarise(meanResultValue = mean(ResultMeasureValue, na.rm = TRUE)) %>%
                                       pivot_wider(names_from = Variable, 
                                                   values_from = meanResultValue) %>%
                                       mutate(Month = month(ActivityStartDate), 
                                              Year = year(ActivityStartDate)) %>%
                                       filter(Month %in% seq(5, 10, 1)) %>%   
                                       filter(Depth_m <= 1)

#Now plotthe clorophyll-TP stressor-response relationship, with a linear model of the relationship and adjusted axes
Chl_TP_stressorResponse.plot <- ggplot(NCdata_summary, aes(x = TP_mgL, y = Chla_ugL)) +
                                geom_point(alpha = 0.4, color = "navyblue") +
                                geom_smooth(method = lm, se = FALSE, color = "indianred") + 
                                scale_x_log10() + 
                                scale_y_log10() + 
                                labs(title = "Chlorophyll-A - Total Phosphorus stressor response relationship",
                                     x = "Total Phosphorus (mg/L)",
                                     y = "Chlorophyll-A (ug/L)")
Chl_TP_stressorResponse.plot
```
...
> This stressor repsonse relationship indicates that the chlorophyll-A concentration in the epilimnion of NC lakes increases as the TP concentration increases. However, the spread of points around the regression line indicates that there is not a perfect correlation between phosophorus and chlorophyll across all the lakes and reservoirs of North Carolina. Rather, the severity of adverse impacts arising from chlorophyll inputs varies. This could arise from the existence of other factors that either limit or augment algal (and, accordingly, chlorophyll) concentrations in the lakes. These factors could include other nutrient concentrations (likely to be higher in the piedmont due to agricultural land uses), temperature (likely to be lower in the mountains due to elevation), and overall ecosystem structure (with macrophyte-heavy lakes having a lower concentration of chlorophyll relative to their phosohorus content than others). This last explanation could be the most slaient in consdiering the differences between natural lakes and reservoirs. The rate of through-flow of water could also affect the growth response of algae relative to phophorus, with the higher possibility for high rates of flow through lakes at elevation perhaps reducing the chlorophyll concentration relative to phosphous in mountain lakes, and the manual control of reservoirs affecting the patterns of reservoirs. 