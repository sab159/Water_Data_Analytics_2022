...
```{r}
LAGOSMN <- LAGOSjoined %>% left_join(.,LAGOSdata$hu4) %>% 
...
ggplot(LAGOSMN, aes(x = tp, y = chla)) +
       geom_point(alpha = 0.1) +
       geom_smooth(method = "lm", se = FALSE) +
       facet_wrap(vars(hu4), ncol = 3) +
       scale_y_log10() +
       scale_x_log10() +
       labs(x = expression("TP ("*mu*"g/L)"), y = expression("Chl a ("*mu*"g/L"))

StressorResponseRelationship <- lm(data=LAGOSMN, chla ~ log10(tp) * hu4)
summary(StressorResponseRelationship)
#indicated different stressor response by classes
```
