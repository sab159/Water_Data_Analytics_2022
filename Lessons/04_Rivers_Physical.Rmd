...
> Climate change will alter the timing & magnitude of precipitation events, as well as the type of precipitation. Temperature changes may also alter evapotranpiration and the amount of water reaching rivers & streams
...
EnoDischarge <- readNWISdv(siteNumbers = "02085070",
                     ...
                     startDate = "", #earliest available period of record
                     endDate = "2021-09-30") #end of water year (Oct to Sept)
...
EnoDischarge <- EnoDischarge %>%
  ...
  filter(WaterYear != "1963") #dropping incomplete initial water year (only 30 days before water year 1964 begins)
...
ggplot(EnoRecurrence, aes(x = RecurrenceInterval, y = PeakDischarge)) +
  geom_point() +
  scale_x_log10() +
  geom_smooth(method = "lm") #can also specify "color = " and whether or not to include error bar (se = FALSE)
#decent fit in the middle, less fit at upper and lower limits


#Linear model, from which we can extract summary
...
> At high discharges, the models diverge, indicating that higher magnitude discharge events are occurring with greater frequency in recent years. 
...
> Use rolling 30 year windows to assess recurrence intervals  
