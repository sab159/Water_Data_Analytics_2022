...
```{r}
HawSummary <- whatNWISdata(siteNumbers = "02096500") #NC
VerdeSummary <- whatNWISdata(siteNumbers = "09504000") #AZ
BitterrootSummary <- whatNWISdata(siteNumbers = "12344000") #MT
SaukSummary <- whatNWISdata(siteNumbers = "05270500") #MN
NehalemSummary <- whatNWISdata(siteNumbers = "14299800") #OR
```
...
> Haw River: HawPoR <- HawSummary %>% mutate(periodOfRecord = end_date - begin_date)
...
# Renaming columns (one method of multiple) - do before binding into a wide dataset
...
# Remove agency_cd (drop with -<col name>)column from each data frame
...
EnoPlot
# Low seasonality, largely driven by distinct precipitation events - Eno
...
VerdePlot <- 
  ...
  theme(plot.title = element_text(margin = margin(b = -10), size = 12), 
        axis.text.x = element_blank(), axis.title.x = element_blank())
VerdePlot
# Ephemeral river - very arid, little flow
...
BitterrootPlot <- 
  ...
  theme(plot.title = element_text(margin = margin(b = -10), size = 12), 
        axis.text.x = element_blank(), axis.title.x = element_blank())
BitterrootPlot
# Mountain west river, with high snowmelt signal (annual spring peak) & high gradient drainage
...
SaukPlot <- 
  ...
  theme(plot.title = element_text(margin = margin(b = -10), size = 12), 
        axis.text.x = element_blank(), axis.title.x = element_blank())         
SaukPlot
# Some seasonal signalling, less flashy than Eno
...
NehalemPlot <- 
  ...
  theme(plot.title = element_text(margin = margin(b = -10), size = 12))
NehalemPlot
# Small watershed, seasonal signals of wet & dry seasons (winter, rather than spring, peak) with moderation from Pacific ocean. Some flashiness from land use conversion

CombinedPlot <- 
  plot_grid(EnoPlot, VerdePlot, BitterrootPlot, SaukPlot, NehalemPlot, 
            ncol = 1, rel_heights = c(1, 1, 1, 1, 1.2), #use relative heights to make last one slight larger, as labels shrank it
...
BitterrootDischarge.Historical <- readNWISdv(siteNumbers = "12344000",
                     parameterCd = "00060", # discharge (ft3/s)
                     startDate = "") #blank start date grabs full period of record
