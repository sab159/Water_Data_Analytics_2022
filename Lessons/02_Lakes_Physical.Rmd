...
What are the physical properties of lakes?
  ...
  ANSWER: Temperature, depth, light transmission, etc. 
...
# Install packages of interest - MUST COMMENT FOR Rmd to knit!!!
...
# ANSWER: does not let us see the logic of the sampling (ie. half meter increments)

# QA the one data point at 20 m depth. 
Pauldata <- Pauldata %>%
  filter(depth < 20) #omit the anamolous data point 
...
ggplot(subset(Pauldata, year4 == 1993), 
       ...
       scale_y_reverse() +
       scale_color_viridis_c() +
       labs(x = expression("Temperature "(degree*C)), y = "Depth (m)", color = "DOY")                  #'expression()' clause to get degree symbol

# Now, make a plot as above but with dissolved oxygen saturation instead of temperature.
ggplot(subset(Pauldata, year4 == 1993), #use 'subset()' to get a subset without creating distinct dataset
...
# exercise: put the legend on top and relabel the aesthetics
  ...
  labs(x = "Day number", y = "Thermocline depth (m)") + 
  scale_y_reverse()
...
# exercise: relabel the aesthetics
  labs(x = "Day number", y = expression("Temperature " (degree*C)))
