...
> The noise makes it difficult to detect any strong trend over time. The seasonality of the discharge could be an important consideration for making comparisons and precdictions. The occurrence of high peaks appears to be increasing, which could be a systemic or random trend. It is harder to discern if lower high peaks are also following this trend. Any specific events that may have impacted discharge (ex. changes to hydrologic regime, etc.) oculd be wroth noting. 

> Changing to a log scale could also show more discernibly if changes are occurring: 
...
> In the above example, a linear trend isn't a great fit. A linear regression model may be better for datasets where seasonal variability is not present or change occurs more gradually. 
...
# Double brackets as coding convention for accessing 4th row
#data must be equal interval and without NAs
# Frequency is occurrences per year
...
# side bar shows difference in scale between datasets
# lots of high flow events in the remainder (not in the trend)
...
> Lack or presence of negative trend values is an artifact of the methodology. 
...
* **Piecewise constant**: "Nearest neighbor" - fill with preceding value
* **Linear**: "Connect the dots" - connect neighboring values with linear model
* **Spline**: Quadratic formulas to define values
...
#need to pull in values - created as a separate dataframe
```
...
```{r}
# Create time series
   cond.ts <- ts(NeuseCond_monthly$Conductivity, frequency = 12)

# Decompose & plot the time series
   cond.decomp <- stl(cond.ts, s.window = "periodic")
   plot(cond.decomp)

   cond.comp <- as.data.frame(cond.decomp$time.series[,1:3])
   cond.comp <- mutate(cond.comp,
...
```
 ...
 > The magnitude of the seasonality is about one tenth of the total variability present in the data. The model suffers at the 6 year gap in the data, as a linear model is too simple. A splicing of modelled data based on the decomposition could more accurately model the missing data. 
